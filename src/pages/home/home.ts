  ...
  dogsGET(comps.searchBar.value || String(actualPage))
    .then((data: Dog[]) => {
      data.forEach((dog: Dog) => {
        const dogLabel = document.createElement("a") as HTMLAnchorElement;
        dogLabel.setAttribute("class", "dogsLabel");
        dogLabel.setAttribute("href", `../dog/dog.html?id=${dog.id}`);
        dogLabel.innerHTML = dog.name;

        const dogLabelDIV = document.createElement("div") as HTMLDivElement;
        dogLabelDIV.setAttribute("class", "dogsLabelDIV");

        const dogFrame = document.createElement("div") as HTMLDivElement;
        dogFrame.setAttribute("class", "frameDOGS");

        const dogImg = document.createElement("div") as HTMLDivElement;
        dogImg.setAttribute("class", "dogsImg");
        dogImg.style.setProperty(
          "background-image",
          `url(${IMAGES_URL}/${dog.reference_image_id}.jpg)`
        );
